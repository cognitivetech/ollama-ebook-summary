...
def split_text_by_headings(text: str, row_headings: List[str]) -> List[Tuple[str, str]]:
    ...
    for line_num, line in enumerate(lines, start=1):
        ...
        logging.debug(f"Processing line {line_num}: '{original_line}'")

        # Attempt to match the line to one of the row_headings
        matched_heading = None
        clean_line = re.sub(r'^\d+(\.\d+)*\s*', '', line).strip()
        logging.debug(f"Cleaned line for matching: '{clean_line}'")

        for heading in row_headings:
            ratio = SequenceMatcher(None, clean_line.lower(), heading.lower()).ratio()
            logging.debug(f"Matching '{clean_line}' with heading '{heading}' | Similarity Ratio: {ratio:.2f}")
            if clean_line.lower() == heading.lower() or ratio > 0.9:
                matched_heading = heading
                found_any_heading = True
                logging.debug(f"Matched heading: '{matched_heading}'")
                break

        if matched_heading is not None:
            finalize_current_section()
            current_heading = matched_heading
            logging.debug(f"Set current_heading to '{current_heading}'")
        else:
            if current_heading is None:
                if row_headings:
                    current_heading = row_headings[0]
                    found_any_heading = True
                    logging.debug(f"No current_heading set. Defaulting to first heading '{current_heading}'")
            current_content_lines.append(line)
            logging.debug(f"Added line to current_content_lines. Total lines now: {len(current_content_lines)}")
